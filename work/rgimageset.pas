...
uses
  ...
  Imaging, ImagingDds, ImagingTypes, ImagingComponents, rgctrl;
...
type
  ...
  public
    ...
    // list of sprites to unpack
    function GetSprite(const aname:string ; astrm:TStream):integer;  // single sprite. to stream
    function GetSprite(      idx  :integer; astrm:TStream):integer;  // single sprite. to stream
    function GetSprite(const aname:string ; var buf:PByte):integer;  // single sprite. to memory buffer
    function GetSprite(      idx  :integer; var buf:PByte):integer;  // single sprite. to memory buffer
    function GetSprite(const aname:string ; apic:TPicture):boolean;  // single sprite. to TPicture
    function GetSprite(      idx  :integer; apic:TPicture):boolean;  // single sprite. to TPicture
    function GetSprite(const aname:string                ):boolean;  // single sprite. to disk file
    function GetSprite(      idx  :integer               ):boolean;  // single sprite. to disk file
    // unrealized yet
...
begin
  ...
  NewImage(arect.Right-arect.Left,arect.Bottom-arect.Top,
...
begin
  ...
  begin
    if ExtractRect(Fitems[i].Name, Rect(
        ...
        FItems[i].XPos+FItems[i].Width,
        FItems[i].YPos+FItems[i].Height)) then inc(result);
  end;
end;
{%ENDREGION Extract}
...
begin
  ...
  result:=ExtractRect(Fitems[idx].Name, Rect(
     FItems[idx].XPos,FItems[idx].YPos,
     FItems[idx].XPos+FItems[idx].Width,
     FItems[idx].YPos+FItems[idx].Height))
end;
