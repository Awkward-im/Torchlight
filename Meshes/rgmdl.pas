...
type
  ...
  TDataCatalog = packed record
    ...
    _offset:int16; // buffer offset
    _number:int16; // block number
  end;
type
  TRGMDL = object
  public
  ...
  WriteLine('  <sharedgeometry vertexcount="'+IntToStr(FVertexCount)+'">');
{
  ...
  begin
    ...
    WriteLine('        <texcoord'+
         ' u="'+FloatToStr(FUVs[i].x)+
        '" v="'+FloatToStr(FUVs[i].y)+'" />');
  ...
  begin
    if FVertices<>nil then // fools proof
    begin
      WriteLine('    <vertexbuffer positions="true">');
      for i:=0 to FVertexCount-1 do
      begin
        WriteLine('      <vertex>');
        WriteLine('        <position'+
    ...
    begin
      WriteLine('    <vertexbuffer normals="true">');
      for i:=0 to FVertexCount-1 do
      begin
        WriteLine('      <vertex>');
        WriteLine('        <normal'+
    ...
    if FUVs<>nil then
    begin
      WriteLine('    <vertexbuffer texture_coord_dimensions_0="2" texture_coords="1">');
      for i:=0 to FVertexCount-1 do
      begin
        WriteLine('      <vertex>');
        WriteLine('        <texcoord'+
...
var
  lblocks,lblock,i,j,lface,lcnt,lcnt1:integer;
...
begin
  ...
  begin
    ...
    for j:=0 to FNumBlock-5 do
    begin
      // Block size=1; type=7; #=j
      LogLn();
      Log(HexStr(FStream.Position,8),IntToStr(j+1)+' UV index');
      FStream.Read(lidx[0],FVertexCount*4);
      ...
      begin
        if ltex<>nil then SetLength(ltex,0);
        SetLength(ltex,lcnt);
      end;
...
begin
  ...
  Log('  normals from (+40)',HexStr(FStream.Position+40,8));
  Log('  texture from (+52)',HexStr(FStream.Position+52,8));
  for i:=0 to FVertexCount-1 do
  begin
    FVertices[i].X:=FStream.ReadFloat();    // X Position, 3 reals per vertex VET_FLOAT3
    FVertices[i].Y:=FStream.ReadFloat();    // Y
    FVertices[i].Z:=FStream.ReadFloat();    // Z

    FStream.ReadFloat();    // Tangent  (X axis if normal is Z) tangent_dimensions=4
    FStream.ReadFloat();    // 
    FStream.ReadFloat();    // 
    FStream.ReadFloat();    // 

    FStream.ReadFloat();    // Binormal (Y axis if normal is Z)
    FStream.ReadFloat();    // 
    FStream.ReadFloat();    // 
...
var
  ...
  i,ltmp:integer;
begin
  if not (aver in [$01,$02,$03, $07,$08,$09, $0D,$0E]) then exit(false);
  ...
  begin
    ...
    //--------------------------------
    Log('>colors?','');
    Log('{00} w' ,Int16(FStream.ReadWord())); // alpha_rejection value?
{
    ...
    if aver>=7 then
      Log('{7+ 1} w' ,Int16(FStream.ReadWord())); // can be -1

    if aver>=13 then
    begin
      Log('{13+ 1} w' ,Int16(FStream.ReadWord())); // 1
      Log('{13+ 2} w' ,Int16(FStream.ReadWord())); // 1
    end;
...
// ambient, diffuse, emissive, specular
    ...
    mtl^.textures[ 5]:=LogTexture('diff',Int32(FStream.ReadDWord()));         // 00 base/diffuse
