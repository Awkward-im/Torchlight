...
function CreateLangTable(const lng:string):boolean;

function TLOpenBase ():boolean;
function TLSaveBase ():integer;
function TLCloseBase(dosave:boolean):boolean;

function CopyToBase  (const data:TTL2Translation; withRef:boolean):boolean;
function LoadFromBase(var   data:TTL2Translation;
    amod:Int64; const lang:string):integer;

function AddToModList(aid:Int64; const atitle:string):integer;
function GetModByName(const atitle:string):Int64;
...
function GetTranslation(aid:integer;
   const atable:string; var adst:Ansistring):boolean;
// get reference info. aid is ref index, result is source index
function GetRef(aid:integer;
     var afile,atag:string; var aline,aflags:integer):integer;

procedure PrepareScanSQL;
...
const
  aBool:array [boolean] of string = ('0','1');
...
begin
  Str(stat.modid,lmod);
  ls:='SELECT count(srcid), count(DISTINCT srcid) FROM ref WHERE modid='+lmod;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          stat.langs[i].lang :=iso639.GetLangIndex(Copy(ltab,7));
...
function AddToList(const atable,avalue:string):integer;
begin
...
begin
  ...
  Str(aid,ls);
  result:=ReturnInt(tldb,
    'SELECT 1 FROM dicmods WHERE id='+ls);
...
begin
  ...
  begin
    Str(amod,lsrc);
    result:=ReturnInt(tldb,
      'SELECT count(DISTINCT srcid) FROM ref WHERE modid='+lsrc)
  end
  else
...
begin
  Str(aid,lsrc);
  result:=ReturnText(tldb,'SELECT src FROM strings WHERE id='+lsrc);
end;

function FindOriginal(const asrc:string):integer;
begin
  result:=ReturnInt(tldb,'SELECT id FROM strings WHERE (src=?1)',asrc);
end;

function AddOriginal(const asrc:string; withcheck:boolean=false):integer;
begin
{
...
function GetTranslation(aid:integer; const atable:string;
    var adst:Ansistring):boolean;
var
  vm:pointer;
  lSQL,lsrc:string;
begin
...
function SetTranslation(aid:integer; const atable:string;
...
begin
  ...
  begin
    ExecuteDirect(tldb,
      ...
      lsrc+', ?1, '+aBool[apart]+');',adst);
  end
  else
    ExecuteDirect(tldb,
      'UPDATE '+atable+
      ' SET dst=?1, part='+aBool[apart]+
      ' WHERE srcid='+lsrc,adst);
end;

function AddTranslation(aid:integer; const atable:string;
    const adst:Ansistring; apart:boolean):integer;
var
  lsrc:string;
  lstat:integer;
...
var
  lline,lmod,lsrc,ldir,lfile,ltag:string;
  lflag:integer;
begin
  result:=0;
...
begin
  ...
  begin
    ...
    begin
      ...
      Str(AddToList('filter',FilteredString(lsrc)),lid);
...
function AddScanString(dummy:pointer; const astr:PWideChar;
      const afile, atag:AnsiString; aline:integer):integer;
begin
