{TODO: implement AddPath to add just directory}
...
var
  idx:integer;
begin
  idx:=SearchFile(apath);
  if idx>=0 then
  begin
    if Assigned(OnUpdate) then OnUpdate(idx,act_reset);
    ClearElement(PUpdateFileInfo(Files[idx]));
    DeleteFile(idx);
  end;
end;
...
begin
  idx:=AddFile(apath);
  result:=PUpdateFileInfo(Files[idx]);
  ClearElement(result);
  with result^ do
  begin
    data :=adata;
  ...
  if Assigned(OnUpdate) then OnUpdate(idx,act_data);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      act  :=act_file;
      if Assigned(OnUpdate) then OnUpdate(idx,act_file);
    end;
  end
  else
  begin
