...
type
  ...
  public
    ...
    constructor Create(      aold :PByte ; anew:PByte); overload;

    function UnicodeText:PWideChar;
  end;
...
function TCompareForm.UnicodeText:PWideChar;
var
  slold:TStrings;
begin
  GetPureText(seold,slold);
  result:=StrToWide(slold.Text);
  slold.Free;
end;

constructor TCompareForm.Create(aold:PByte; anew:PByte);
var
...
begin
  ...
  case GetSourceEncoding(aold) of
    tofSrcUTF8: begin
      pc:=PWideChar(aold);
      if (PDword(aold)^ and $00FFFFFF)=SIGN_UTF8 then inc(PAnsiChar(pc),3);
      slold.Text:=PAnsiChar(pc);
    end;

    tofSrcWide: begin
      pc:=PWideChar(aold);
      if ORD(pc^)=SIGN_UNICODE then inc(pc);
      slold.Text:=WideToStr(pc);
    end;
  else
  end;

  case GetSourceEncoding(anew) of
    tofSrcUTF8: begin
      pc:=PWideChar(anew);
      if (PDword(anew)^ and $00FFFFFF)=SIGN_UTF8 then inc(PAnsiChar(pc),3);
      slnew.Text:=PAnsiChar(pc);
    end;
