...
begin
  ...
  lSQL:='INSERT INTO recipes (id, name, title, modid) VALUES ('+sid+', '''+aname+''',"'+atitle+
    '", '+smodid+')';
...
//        write(' name ',name );
      ...
      if CompareWide(p^.children^[i].name,'GUID') then
      begin
        lid:=p^.children^[i].asInteger64;
//        write(' id ',lid);
      end;
      if CompareWide(p^.children^[i].name,'RESULT') then
      begin
