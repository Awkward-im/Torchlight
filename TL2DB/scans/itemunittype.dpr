...
begin
  ...
  if lid<>'' then
  begin
    if (lut='') and (lbase<>'') then
      lut:=GetBaseUnitType(lbase);

    lSQL:='UPDATE items SET unittype='+FixedText(lut)+' WHERE id='+lid;
